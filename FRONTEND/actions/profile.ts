...
/**
 * Saves a business to user's saved list
 */
export async function saveBusinessForUser(userId: string, businessId: string): Promise<{ success: boolean; error?: string }> {
  try {
    const supabase = await createClient()
    ...
    if (error) {
      logger.error("saveBusinessForUser: Error saving business", error, { userId, businessId })
    ...
    logger.error("saveBusinessForUser: Unexpected error", error)
    return { success: false, error: "Failed to save business" }
  }
}

/**
 * Removes a business from user's saved list
 */
export async function removeSavedBusiness(userId: string, businessId: string): Promise<{ success: boolean; error?: string }> {
  try {
    const supabase = await createClient()
    ...
    if (error) {
      logger.error("removeSavedBusiness: Error removing business", error, { userId, businessId })
      return { success: false, error: error.message }
    }
    ...
    logger.error("removeSavedBusiness: Unexpected error", error)
    return { success: false, error: "Failed to remove saved business" }
  }
}
