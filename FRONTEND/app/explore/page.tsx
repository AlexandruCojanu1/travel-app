...
// Map SwipeCategory to recommendation category
const CATEGORY_MAP: Record<SwipeCategory, "hotel" | "restaurant" | "activity"> = {
    hotel: "hotel",
...
function ExploreContent() {
    ...
    const { tripDetails, budget, updateTripMetadata } = useTripStore()
    ...
    const recommendationCategory = CATEGORY_MAP[currentCategory]
    const { recommendations, loading: recommendationsLoading } = useRecommendations(recommendationCategory)
    ...
    useEffect(() => {
        async function loadBusinesses() {
            ...
            try {
                ...
                // Sort by recommendations if available, otherwise shuffle
                Object.keys(categorized).forEach(key => {
                    const category = key as SwipeCategory
                    const recCategory = CATEGORY_MAP[category]
                    
                    // If we have trip details and budget, we'll sort by recommendations later
                    // For now, shuffle as fallback
    ...
    // Update current businesses when category changes and apply smart sorting
    useEffect(() => {
        let businesses = [...allBusinesses[currentCategory]]
        
        // Apply smart budget recommendations if available
        if (recommendations.length > 0 && tripDetails && budget) {
            // Create a map of business IDs to scores
            const scoreMap = new Map<string, number>()
            recommendations.forEach(rec => {
                scoreMap.set(rec.business.id, rec.score)
            })
            
            // Sort businesses by recommendation score (highest first)
            businesses.sort((a, b) => {
            ...
            // Fallback: shuffle if no recommendations
            businesses = businesses.sort(() => Math.random() - 0.5)
        }
        
        setCurrentBusinesses(businesses)
    }, [currentCategory, allBusinesses, recommendations, tripDetails, budget])
    ...
    const handleLike = useCallback((business: MapBusiness) => {
        if (currentCategory === 'hotel') {
            ...
            // Save hotel coordinates for restaurant recommendations
