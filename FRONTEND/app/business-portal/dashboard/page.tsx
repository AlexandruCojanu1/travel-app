...
export default function BusinessPortalDashboard() {
  ...
  const loadBusinesses = async () => {
    ...
    try {
      ...
      // Use backend API - get user's owned businesses
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000/api'
      
      const endpoint = `${apiUrl}/businesses/user/owned`
      logger.log("Business Dashboard: Fetching from", endpoint)
      logger.log("Business Dashboard: API URL", apiUrl)
      ...
      try {
        ...
        clearTimeout(timeoutId)
        logger.error("Business Dashboard: Network error", { 
          error: fetchError, 
        ...
        if (fetchError.name === 'AbortError') {
          throw new Error('Request timeout: Backend server may be slow or unresponsive. Please check if the backend is running on port 4000.')
        }
        if (fetchError.message?.includes('Failed to fetch') || fetchError.message?.includes('NetworkError')) {
          throw new Error(`Cannot connect to backend at ${apiUrl}. Please ensure the backend server is running on port 4000.`)
        }
        throw new Error(`Network error: ${fetchError.message || 'Failed to connect to backend'}`)
      }

      if (!response.ok) {
        const errorText = await response.text()
        logger.error("Business Dashboard: API error", { status: response.status, error: errorText })
      ...
      logger.log("Business Dashboard: Result", result)

      // Backend returns { success: true, businesses: [], total: number }
      const businessesList = result.businesses || []
