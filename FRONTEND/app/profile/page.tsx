...
import { getUserPassport, getUserAchievements, getUserQuests } from "@/actions/gamification"
...
import { QuestsList } from "@/components/features/gamification/quests-list"
import type { UserPassport, AchievementWithStatus, UserQuest, Quest } from "@/types/gamification.types"
...
export default function ProfilePage() {
  ...
  // Gamification State
  const [passportData, setPassportData] = useState<UserPassport | null>(null)
  const [achievementsData, setAchievementsData] = useState<{ achievements: AchievementWithStatus[], unlockedIds: string[] } | null>(null)
  const [questsData, setQuestsData] = useState<{ activeQuests: UserQuest[], completedQuests: UserQuest[], availableQuests?: Quest[] } | null>(null)
  ...
  return (
    <div className="w-full max-w-screen-xl mx-auto px-4 md:px-6 lg:px-8 space-y-6 pb-20">
      ...
      <div className="space-y-3">
        ...
        {achievementsData && (
          <div className="space-y-4">
            <h2 className="text-lg font-bold text-mova-dark px-2">Achievements</h2>
        ...
        {questsData && (
          <div className="space-y-4">
            <h2 className="text-lg font-bold text-mova-dark px-2">Quest-uri</h2>
            <QuestsList
              activeQuests={questsData.activeQuests}
              completedQuests={questsData.completedQuests}
              availableQuests={questsData.availableQuests || []}
            />
          </div>
        )}
