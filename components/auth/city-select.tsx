...
export function CitySelect({ value, onChange, error }: CitySelectProps) {
  ...
  useEffect(() => {
    if (value && cities.length > 0) {
      ...
      console.log("CitySelect: value changed, selected city:", city?.name || "none")
    } else if (!value) {
  ...
  return (
    <div className="relative w-full z-50">
      ...
      {/* Click outside to close - MUST be before dropdown for proper z-index stacking */}
      {isOpen && (
        <div
      ...
      <button
        ...
        className={cn(
          "w-full h-14 px-4 rounded-xl border-2 bg-white text-left transition-colors flex items-center justify-between relative z-[50]",
          error
                ...
                filteredCities.map((city) => (
                  <button
                    ...
                    onMouseDown={(e) => {
                      e.stopPropagation()
                      e.preventDefault()
                      console.log("CitySelect: MouseDown on city:", city.name)
                      handleSelect(city)
                    }}
                    onClick={(e) => {
                      e.stopPropagation()
                      e.preventDefault()
                      console.log("CitySelect: Click on city:", city.name)
                      handleSelect(city)
                    }}
                    className={cn(
                      "w-full px-4 py-3 text-left hover:bg-blue-50 active:bg-blue-100 transition-colors flex items-center gap-3 border-b border-slate-100 last:border-0 cursor-pointer select-none",
